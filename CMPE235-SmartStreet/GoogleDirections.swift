...
// MARK: GoogleDirectionsDelegate protocol
@objc public protocol GoogleDirectionsDelegate {
	/**
	Notifies the delegate that an API request is going to be sent.
	
	- parameter googleDirections: The `GoogleDirections` object issuing the request
	- parameter requestURL: The URL that is going to be called
	- returns: `true` if the request should be sent, `false` otherwise
	*/
	optional func googleDirectionsWillSendRequestToAPI(googleDirections: GoogleDirections, withURL requestURL: NSURL) -> Bool
	/**
	...
	*/
	optional func googleDirectionsDidSendRequestToAPI(googleDirections: GoogleDirections, withURL requestURL: NSURL)
	/**
	Notifies the delegate that (yet unparsed) response data has been received from the API.
	
	- parameter googleDirections: The `GoogleDirections` object that issued the request
	- parameter data: An `NSData` object containing raw data received from the API
	*/
	optional func googleDirections(googleDirections: GoogleDirections, didReceiveRawDataFromAPI data: NSData)
	/**
	...
	*/
	optional func googleDirectionsRequestDidFail(googleDirections: GoogleDirections, withError error: NSError)
	/**
	Notifies the delegate that a response has been successfully received and parsed from the API.
	
	- parameter googleDirections: The `GoogleDirections` object that issued the request
	- parameter apiResponse: A list of `GoogleDirectionsRoute` objects containing all relevant data received from the API
	*/
	optional func googleDirections(googleDirections: GoogleDirections, didReceiveResponseFromAPI apiResponse: [GoogleDirectionsRoute])
}

// MARK: GoogleDirectionsSteppable protocol
protocol GoogleDirectionsSteppable {
	var steps: [GoogleDirectionsRouteLegStep] { get set }
}

public typealias GoogleDirectionsRequestCompletionBlock = (GoogleDirectionsResponse) -> Void

// MARK: GoogleDirections class
public class GoogleDirections: NSObject {
	
	// MARK: Private class variables and consants
	
	private static let errorDomain = "GoogleDirectionsErrorDomain"
	...
	public var mode: GoogleDirectionsMode?
	...
	public var featuresToAvoid = Set<GoogleDirectionsFeature>()
	/// Specifies the unit system to use when displaying results
	public var units: GoogleDirectionsUnit?
	...
	public var transitModes = Set<GoogleDirectionsTransitMode>()
	/// Specifies preferences for transit routes
	public var transitRoutingPreference: GoogleDirectionsTransitRoutingPreference?
	...
	public var delegate: GoogleDirectionsDelegate?
	...
	*/
	public var directionsAPIRequestURL: NSURL? {
		...
		if let f = from, t = to {
			...
			var preparedRequest = "\(GoogleDirections.apiBaseURL)?key=\(GoogleDirections.apiKey)&origin=\(f)&destination=\(t)"
	...
	/**
	Creates a new instance of `GoogleDirections` with the specified API key.
	
	- parameter apiKey: Your own API key, provided by Google
	*/
	public init(apiKey: String) {
		super.init()
		GoogleDirections.apiKey = apiKey
	}
	
	/**
	Creates a new instance of `GoogleDirections` with the specified API key, origin and destination.
	...
	*/
	public convenience init(apiKey: String, from: Location, to: Location) {
	...
	*/
	public func calculateDirections(completion: GoogleDirectionsRequestCompletionBlock)
	{
		...
		if let requestURL = directionsAPIRequestURL {
			...
			if runQuery {
				NSURLSession.sharedSession().dataTaskWithURL(requestURL, completionHandler: { (data, response, error) -> Void in
					...
					if error == nil {
						var response = [GoogleDirectionsRoute]()
						...
						do {
							if let json = try NSJSONSerialization.JSONObjectWithData(data!, options: []) as? [String: AnyObject] {
								...
								if let st = json["status"] as? String {
									let status = GoogleDirectionsError.errorFromStatus(st)
									...
									if status.failed {
										...
										if let routes = json["routes"] as AnyObject? as? [[String: AnyObject]] {
											...
											for r in routes {
												...
												let currentRoute = GoogleDirectionsRoute()
												...
												if let fare = r["fare"] as? [String: AnyObject] {
													currentRoute.fare = GoogleDirectionsRouteFare(currency: fare["currency"] as? String, value: fare["value"] as? Double)
												}
												// Overview polyline
												...
												if let legs = r["legs"] as? [[String: AnyObject]] {
													...
													for l in legs {
														...
														var currentLeg = GoogleDirectionsRouteLeg()
														...
														if let distance = l["distance"] as? [String: AnyObject] {
															currentLeg.distance = GoogleDirectionsDistance(distance: distance["value"] as? CLLocationDistance, description: distance["text"] as? String)
														}
														// Duration
														if let duration = l["duration"] as? [String: AnyObject] {
															currentLeg.duration = GoogleDirectionsDuration(duration: duration["value"] as? NSTimeInterval, description: duration["text"] as? String)
														}
														// Duration in traffic
														if let durationT = l["duration_in_traffic"] as? [String: AnyObject] {
															if let dTText = durationT["text"] as? String, dTValue = durationT["value"] as? NSTimeInterval {
																currentLeg.durationInTraffic = GoogleDirectionsDuration(duration: dTValue, description: dTText)
															}
														}
														// Departure time
														if let departure = l["departure_time"] as? [String: AnyObject] {
															currentLeg.departureTime = GoogleDirectionsTime(description: departure["text"] as? String, timeZone: nil, timestamp: departure["value"] as? NSTimeInterval)
														...
														if let arrival = l["arrival_time"] as? [String: AnyObject] {
															currentLeg.arrivalTime = GoogleDirectionsTime(description: arrival["text"] as? String, timeZone: nil, timestamp: arrival["value"] as? NSTimeInterval)
						...
						completion(GoogleDirectionsResponse.Error(self, error!))
						return
					}
				}).resume()
				// Notify delegate
	...
	*/
	public func openInGoogleMaps(center center: CLLocationCoordinate2D?, mapMode: GoogleMapsMode?, view: Set<GoogleMapsView>?, zoom: UInt?, callbackURL: NSURL?, callbackName: String?, fallbackToAppleMaps: Bool = true) -> Bool {
		// Ensure both origin and destination are set
		if let f = from, t = to {
			// Ensure there is actually something specified for both origin and destination addresses
			if f.isSpecified() && t.isSpecified() {
				// Prepare the base URL parameters with provided arguments
				var params = GoogleDirections.handleGoogleMapsURL(center: center, mapMode: mapMode, view: view, zoom: zoom)
				...
				if let url = GoogleDirections.buildGoogleMapsURL(params: params, callbackURL: callbackURL, callbackName: callbackName) {
					UIApplication.sharedApplication().openURL(url)
					return true
				} else {
					// Apply fallback strategy
					if fallbackToAppleMaps {
						var params = GoogleDirections.handleAppleMapsURL(center: center, mapMode: mapMode, view: view, zoom: zoom)
						params.append("saddr=\(f.description.stringByAddingPercentEncodingWithAllowedCharacters(NSCharacterSet.URLQueryAllowedCharacterSet())!)")
						params.append("daddr=\(t.description.stringByAddingPercentEncodingWithAllowedCharacters(NSCharacterSet.URLQueryAllowedCharacterSet())!)")
	...
	private func prepareError(code: GoogleDirectionsError) -> NSError {
		return NSError(domain: GoogleDirections.errorDomain, code: code.rawValue, userInfo: [NSLocalizedDescriptionKey: code.description])
	}
	
	private func handleSteps(jsonData: [String: AnyObject]) -> [GoogleDirectionsRouteLegStep] {
		var results = [GoogleDirectionsRouteLegStep]()
		if let steps = jsonData["steps"] as? [[String: AnyObject]] {
			for s in steps {
				// Loop through steps
				let currentStep = GoogleDirectionsRouteLegStep()
				...
				currentStep.travelMode = GoogleDirectionsMode.modeFromLabel((s["travel_mode"] as? String) ?? "")
				// Distance
				if let distance = s["distance"] as? [String: AnyObject] {
					currentStep.distance = GoogleDirectionsDistance(distance: distance["value"] as? CLLocationDistance, description: distance["text"] as? String)
				}
				// Duration
				if let duration = s["duration"] as? [String: AnyObject] {
					currentStep.duration = GoogleDirectionsDuration(duration: duration["value"] as? NSTimeInterval, description: duration["text"] as? String)
				}
				// Polyline
				...
				if let transitDetails = s["transit_details"] as? [String: AnyObject] {
					...
					currentStep.transitDetails = GoogleDirectionsRouteLegStepTransitDetails()
					...
					if let departureStop = transitDetails["departure_stop"] as? [String: AnyObject] {
						if let dsName = departureStop["name"] as? String, dsLocation = departureStop["location"] as? [String: CLLocationDegrees] {
							if let dslLat = dsLocation["lat"], dslLng = dsLocation["lng"] {
								currentStep.transitDetails?.departureStop = GoogleDirectionsStop(description: dsName, location: CLLocationCoordinate2DMake(dslLat, dslLng))
							}
						}
					}
					// Arrival stop
					if let arrivalStop = transitDetails["arrival_stop"] as? [String: AnyObject] {
						if let asName = arrivalStop["name"] as? String, asLocation = arrivalStop["location"] as? [String: CLLocationDegrees] {
							if let aslLat = asLocation["lat"], aslLng = asLocation["lng"] {
								currentStep.transitDetails?.arrivalStop = GoogleDirectionsStop(description: asName, location: CLLocationCoordinate2DMake(aslLat, aslLng))
							}
						}
					}
					// Departure time
					if let departure = transitDetails["departure_time"] as? [String: AnyObject] {
						currentStep.transitDetails?.departureTime = GoogleDirectionsTime(description: departure["text"] as? String, timeZone: nil, timestamp: departure["value"] as? NSTimeInterval)
						if let dTZ = departure["time_zone"] as? String {
					...
					if let arrival = transitDetails["arrival_time"] as? [String: AnyObject] {
						currentStep.transitDetails?.arrivalTime = GoogleDirectionsTime(description: arrival["text"] as? String, timeZone: nil, timestamp: arrival["value"] as? NSTimeInterval)
						if let aTZ = arrival["time_zone"] as? String {
					...
					if let line = transitDetails["line"] as? [String: AnyObject] {
						...
						currentStep.transitDetails?.line = GoogleDirectionsTransitLine()
						...
						if let agencies = line["agencies"] as? [[String: String]] {
							for a in agencies {
								currentStep.transitDetails?.line?.agencies.append(GoogleDirectionsTransitAgency(name: a["name"], url: NSURL(string: (a["url"] ?? "")), phone: a["phone"]))
							}
						}
						// Vehicle
						if let vehicle = line["vehicle"] as? [String: String] {
							if let vName = vehicle["name"], vType = vehicle["type"], vIconURL = NSURL(string: vehicle["icon"] ?? "") {
								currentStep.transitDetails?.line?.vehicle = GoogleDirectionsTransitLineVehicle(name: vName, type: GoogleDirectionsVehicleType.vehicleTypeFromValue(vType), icon: nil)
	...
	internal class func handleGoogleMapsURL(center center: CLLocationCoordinate2D?, mapMode: GoogleMapsMode?, view: Set<GoogleMapsView>?, zoom: UInt?) -> [String] {
		var params = [String]()
		if let loc = center {
	...
	internal class func handleAppleMapsURL(center center: CLLocationCoordinate2D?, mapMode: GoogleMapsMode?, view: Set<GoogleMapsView>?, zoom: UInt?) -> [String] {
		var params = [String]()
		if let loc = center {
	...
	internal class func buildGoogleMapsURL(var params params: [String], callbackURL: NSURL?, callbackName: String?) -> NSURL? {
		if GoogleDirections.canOpenInGoogleMaps {
