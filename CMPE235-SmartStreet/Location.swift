...
public enum Location {
	...
	*/
	public func openInGoogleMaps(mapMode mapMode: GoogleMapsMode?, view: Set<GoogleMapsView>?, zoom: UInt?, callbackURL: NSURL?, callbackName: String?, fallbackToAppleMaps: Bool = true) -> Bool {
		// Prepare the base URL parameters with provided arguments
		let params = GoogleDirections.handleGoogleMapsURL(center: centerCoordinate, mapMode: mapMode, view: view, zoom: zoom)
		...
		if let url = GoogleDirections.buildGoogleMapsURL(params: params, callbackURL: callbackURL, callbackName: callbackName) {
			...
			if fallbackToAppleMaps {
				let params = GoogleDirections.handleAppleMapsURL(center: centerCoordinate, mapMode: mapMode, view: view, zoom: zoom)
	...
	- returns: `true` if opening in the Google Maps is available, `false` otherwise
	*/
	public func searchInGoogleMaps(query: String, mapMode: GoogleMapsMode?, view: Set<GoogleMapsView>?, zoom: UInt?, callbackURL: NSURL?, callbackName: String?, fallbackToAppleMaps: Bool = true) -> Bool {
		// Prepare the base URL parameters with provided arguments
		var params = GoogleDirections.handleGoogleMapsURL(center: centerCoordinate, mapMode: mapMode, view: view, zoom: zoom)
		...
		if let url = GoogleDirections.buildGoogleMapsURL(params: params, callbackURL: callbackURL, callbackName: callbackName) {
			...
			// Apply fallback strategy
			if fallbackToAppleMaps {
				var params = GoogleDirections.handleAppleMapsURL(center: centerCoordinate, mapMode: mapMode, view: view, zoom: zoom)
				params.append("q=\(query.stringByAddingPercentEncodingWithAllowedCharacters(NSCharacterSet.URLQueryAllowedCharacterSet())!)")
				let p = (params.count > 0) ? "?" + params.joinWithSeparator("&") : ""
				UIApplication.sharedApplication().openURL(NSURL(string: "https://maps.apple.com/\(p)")!)
				return true
